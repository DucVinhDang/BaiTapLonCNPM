<?php
	class Chucdanh_db_manager {
		...
		function insert(Chucdanh $new_chucdanh) {
			...
			$new_is_delete = $new_chucdanh->get_is_delete();

			$queryCommand = "INSERT INTO qlgv_chucdanh(cd_ten, cd_mota, is_delete) VALUES ('" . $new_cd_ten . "','" . $new_cd_mota . "', '" . $new_is_delete . "')";
		...
		function edit(Chucdanh $edit_chucdanh) {
			...
			$new_is_delete = $edit_chucdanh->get_is_delete();

			$queryCommand = "UPDATE qlgv_chucdanh SET cd_ten = '" . $new_cd_ten . "', cd_mota = '" . $new_cd_mota . "', is_delete = '" . $new_is_delete . "' WHERE cd_id = " . $cd_id;
			$queryResult = $this->db_core->db_query($queryCommand);
