<?php
	class Bomon_db_manager {
		...
		function insert(Bomon $new_bomon) {
			...
			$new_is_delete = $new_bomon->get_is_delete();

			$queryCommand = "INSERT INTO qlgv_bomon(bm_ten, bm_mota, k_id, is_delete) VALUES ('" . $new_bm_ten . "','" . $new_bm_mota . "', '" . $new_k_id . "', '" . $new_is_delete . "')";
		...
		function edit(Bomon $edit_bomon) {
			...
			$new_is_delete = $edit_bomon->get_is_delete();

			$queryCommand = "UPDATE qlgv_bomon SET bm_ten = '" . $new_bm_ten . "', bm_mota = '" . $new_bm_mota . "', k_id ='" . $new_k_id . "', is_delete = '" . $new_is_delete . "' WHERE bm_id = " . $bm_id;
			$queryResult = $this->db_core->db_query($queryCommand);
